...
static void chunkstart(Chunker * c);
...
/*
 * Prepare the Chunker for input.
 */
static void
chunkstart(Chunker * c)
{
	uint32_t i;
...
Chunker *
chunkinit(uint32_t meanlen, uint32_t maxlen)
{
	Chunker * c;
	...
	/*
	 * Allocate memory for Chunker structure.
	 */
	c = malloc(sizeof(*c));
	if (c == NULL)
		return (NULL);
...
int
chunkwrite(Chunker * c, uint8_t * buf, size_t buflen)
{
	uint32_t htpos;
	uint32_t yka_tmp;
	size_t i;
...
main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		if (!rd)
			break;
		chunkwrite(chunker, (uint8_t*)iobuf, rd);
	}
	chunkend(chunker);
